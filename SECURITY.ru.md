...
| --- | --- | --- | --- |
| Trivy (#12) | `exit-code: 0` (только отчёт) | Spring Boot **2.6.7 — EOL**, CVE чинятся только апгрейдом фреймворка; блокировка билда сделала бы CI вечно красным. | Обновить Spring Boot, затем `exit-code: 1`. |
| Свежесть рантайма | Оставлен Spring Boot **2.6.7** (EOL); Java 18 → **17 LTS** | Минимальный хардинг готовой практики. Java 18 — EOL; 17 LTS в матрице Boot 2.6. | Перейти на Spring Boot 3.x + Java 21 LTS. |
...
- [ ] Апгрейд Spring Boot 2.6.7 → 3.x и Java 17 → 21 (убирает большинство находок Trivy)
